...
void processMove(void)
{
  ...
  x = Serial.parseInt(SKIP_NONE);
  y = Serial.parseInt(SKIP_NONE);
...
void moveRel(int dx, int dy)
{
  ...
  if (dx > dy) {
    for (i=0; i<dx; ++i)
    {
      ...
      if penPos.x>0 and penPos.x<MAX_X myStepperX.step(sx);
      over+=dy;
      if (over>=dx) {
        over-=dx;
        penPos.y+=sy
        // Only step the motor if not beyond the limits
        if penPos.y>0 and penPos.y<MAX_Y myStepperY.step(sy);
      }
      delay(StepDelay);
    }
  }
